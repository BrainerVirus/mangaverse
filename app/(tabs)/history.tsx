	...
	return (
		<View className="bg-background flex-1">
			...
			<ScrollView className="flex-1 px-5 pt-6" contentInsetAdjustmentBehavior="automatic" contentContainerStyle={{ paddingBottom: tabBarPadding }}>
				...
				{entries.length === 0 ? (
					<View className="border-border/30 bg-card/80 rounded-box mt-6 border p-6">
					...
					<View className="mt-6 gap-4 pb-12">
						<View className="border-border/30 bg-card/70 rounded-box border p-5">
							<View className="flex-row items-center justify-between">
								...
								<View className="border-border/40 bg-background/60 rounded-badge border px-3 py-2">
									<Text className="text-preset-1 text-muted tracking-[0.2em] uppercase">History</Text>
								</View>
							</View>
							<View className="mt-4 gap-3">
								{entries.map((entry) => (
									<View key={entry.id} className="border-border/30 bg-card/70 rounded-box border p-5">
