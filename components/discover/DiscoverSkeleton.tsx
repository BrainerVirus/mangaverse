...
export function DiscoverSkeleton({ gap, cardWidth, heroWidth }: DiscoverSkeletonProps) {
	...
	return (
		<View className="mt-6 gap-6">
			<View>
				<Animated.View
					style={[{ width: heroWidth, height: heroHeight, borderRadius: 26 }, shimmerStyle]}
					className="bg-card"
				/>
			</View>
				...
				{[0, 1].map((sectionIndex) => (
					<View key={`section-${sectionIndex}`}>
						<View className="flex-row items-center justify-between">
							<Animated.View
								style={[{ height: 16, width: sectionTitleWidth, borderRadius: 8 }, shimmerStyle]}
								className="bg-card"
							/>
							<Animated.View
								style={[{ height: 40, width: 40, borderRadius: 16 }, shimmerStyle]}
								className="bg-card"
							/>
						</View>
