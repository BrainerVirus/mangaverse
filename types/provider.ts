...
export interface ProviderContract {
	...
	getDiscoverSectionItems: (sectionId: string, page: number, filters?: Record<string, unknown>) => Promise<ProviderMangaItem[]>;
	search: (query: string, page: number, filters?: Record<string, unknown>) => Promise<ProviderMangaItem[]>;
	getAvailableFilters: () => Promise<ProviderFilter[]>;
	getMangaDetails: (providerMangaId: string) => Promise<ProviderMangaItem>;
	getChapterList: (providerMangaId: string, languages?: string[]) => Promise<ProviderChapter[]>;
