...
export const parseOklch = (value: string) => {
	const match = /oklch\(\s*([0-9.]+)\s+([0-9.]+)\s+([0-9.]+)\s*(?:\/\s*([0-9.]+%?))?\s*\)/i.exec(value);
	if (!match) {
		return null;
	}
	...
	const alpha = match[4];
	if (Number.isNaN(lightness) || Number.isNaN(chroma) || Number.isNaN(hue) || lightness < 0 || lightness > 1) {
		return null;
	}
...
export const oklchToHex = (value: string) => {
	...
	const toSrgb = (channel: number) => (channel <= 0.0031308 ? 12.92 * channel : 1.055 * channel ** (1 / 2.4) - 0.055);
