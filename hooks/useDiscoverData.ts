...
export function useDiscoverData({ providersMap, selectedProviderId, refreshProviders }: UseDiscoverDataOptions): UseDiscoverDataResult {
	...
	useEffect(() => {
		...
		loadSections()
			.catch((err) => setError(err instanceof Error ? err.message : 'Failed to load discover sections'))
			.finally(() => {
				if (loadRequestId.current === requestId) {
					setLoading(false);
					setRefreshing(false);
	...
	const orderedSections = useMemo(() => {
		...
		const ordered = SECTION_ORDER.map((id) => lookup.get(id)).filter((section): section is ProviderDiscoverSection => Boolean(section));
		const remaining = sections.filter((section: ProviderDiscoverSection) => !SECTION_ORDER.includes(section.id));
