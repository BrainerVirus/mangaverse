...
const mapMangaDetails = (html, providerMangaId) => {
...
const provider = {
    ...
    async search(query, page) {
        const pageParam = page > 1 ? `&page=${page}` : "";
        const html = await fetchText(`${baseUrl}/library?title=${encodeURIComponent(query)}${pageParam}`);
        const items = mapLibraryItems(html);
        return items.map((item) => ({
            id: item.id,
            title: item.title,
    ...
    async getChapterList(providerMangaId) {
        const html = await fetchText(`${baseUrl}/library/manga/${providerMangaId}`);
